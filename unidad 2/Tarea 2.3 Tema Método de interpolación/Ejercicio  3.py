...
# Función original: f(x) = sen(x) - x/2
def f(x):
    return np.sin(x) - (x / 2)
...
# Selección de tres puntos adecuados dentro del intervalo [-1, 1]
x0 = -1
x1 = 0
x2 = 1
...
axs[0].plot(np.linspace(x0, x2, 100), f(np.linspace(x0, x2, 100)), label="f(x) = sen(x) - x/2", linestyle='dashed', color='blue')
