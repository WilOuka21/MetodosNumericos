...
def g_prime(x):
    return -np.sin(x)  # Debe cumplir |g'(x)| < 1
...
axs[0].plot(x_vals, y_vals, label=r"$g(x) = \cos(x)$", color="blue")
...
axs[0].legend()
axs[0].grid(True)
axs[0].set_title("Método de Punto Fijo - Ejercicio 3")
...
plt.tight_layout()
plt.savefig("punto_fijo_convergencia_errores_ejercicio_3.png")
plt.show()
